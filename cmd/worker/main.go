...
func main() {
	...
	worker.AddCommand(worker.NewCmd(log).SetName("marketplace-colletion-snapshot-summarized").SetAliases("snapsum").SetUsage("Summarize snapshot for collection").SetRun(snapshotsummarized.Run))
